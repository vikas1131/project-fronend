// __tests__/apiClientEngineer.test.js

let apiClient;
let originalSessionStorage;
let originalLocation;

describe('apiClientEngineer interceptors', () => {
  beforeEach(() => {
    ...
    // Import the axios client module.
    // Adjust the path as necessary for your project structure.
    apiClient = require('./apiClientEngineer').default;
  });
  ...
  describe('Request interceptor', () => {
    ...
    it('should attach token, email, and role headers when available', async () => {
      ...
      window.sessionStorage.getItem.mockImplementation((key) => {
        if (key === 'token') return 'engineerToken';
        if (key === 'email') return 'engineer@example.com';
      ...
      expect(config.headers['Authorization']).toBe('Bearer engineerToken');
      expect(config.headers['X-User-Email']).toBe('engineer@example.com');
      expect(config.headers['X-User-Role']).toBe('engineer');
    });
    ...
    it('should remove email and role headers when they are not available', async () => {
      ...
      // Pre-populate headers to verify they get removed.
  ...
  describe('Response interceptor', () => {
    ...
    it('should not redirect if the error status is not 401', async () => {
      // Simulate an error with status 400.
